...
class CompaniesController extends SecureController{
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($request->search)){
			...
			$search_condition = "(
				...
				companies.address1 LIKE ? OR 
				companies.address2 LIKE ? OR 
				companies.country_id LIKE ? OR 
				companies.tin LIKE ? OR 
				companies.creator LIKE ? OR 
				companies.slug LIKE ? OR 
				companies.status LIKE ? OR 
				companies.created_at LIKE ? OR 
				companies.updated_at LIKE ?
			)";
			$search_params = array(
	...
	function add($formdata = null){
		if($formdata){
			...
			$request = $this->request;
			//fillable fields
			$fields = $this->fields = array("user_id","name","address1","address2","country_id","tin","creator","slug","status","created_at","updated_at");
		...
		$page_title = $this->view->page_title = "Add New Companies";
		$this->render_view("companies/add.php");
	}
	/**
	...
	function edit($rec_id = null, $formdata = null){
		...
		$tablename = $this->tablename;
		 //editable fields
		$fields = $this->fields = array("id","user_id","name","address1","address2","country_id","tin","creator","slug","status","created_at","updated_at");
		if($formdata){
			$postdata = $this->format_request_data($formdata);
			$this->rules_array = array(
				'user_id' => 'required|numeric',
		...
		$page_title = $this->view->page_title = "Edit  Companies";
		if(!$data){
			$this->set_page_error();
		}
		return $this->render_view("companies/edit.php", $data);
	}
	/**
     * Update single field
	 * @param $rec_id (select record by table primary key)
