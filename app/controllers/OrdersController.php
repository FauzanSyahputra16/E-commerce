...
class OrdersController extends SecureController{
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($request->search)){
			...
			$search_condition = "(
				orders.id LIKE ? OR 
				orders.user_id LIKE ? OR 
				orders.carrier_id LIKE ? OR 
				orders.invoice_id LIKE ? OR 
				orders.invoice_date LIKE ? OR 
				orders.delivery_date LIKE ? OR 
				orders.total_discount LIKE ? OR 
				orders.tax LIKE ? OR 
				orders.shipping LIKE ? OR 
				orders.shipping_tax LIKE ? OR 
				orders.subtotal LIKE ? OR 
				orders.total LIKE ? OR 
				orders.creator LIKE ? OR 
				orders.slug LIKE ? OR 
				orders.status LIKE ? OR 
				orders.created_at LIKE ? OR 
				orders.updated_at LIKE ?
			)";
			$search_params = array(
	...
	function add($formdata = null){
		if($formdata){
			...
			$request = $this->request;
			//fillable fields
			$fields = $this->fields = array("user_id","carrier_id","invoice_id","invoice_date","delivery_date","total_discount","tax","shipping","shipping_tax","subtotal","total","creator","slug","status","created_at","updated_at");
			...
			$this->sanitize_array = array(
				...
				'carrier_id' => 'sanitize_string',
				'invoice_id' => 'sanitize_string',
				'invoice_date' => 'sanitize_string',
				'delivery_date' => 'sanitize_string',
				'total_discount' => 'sanitize_string',
				'tax' => 'sanitize_string',
				'shipping' => 'sanitize_string',
				'shipping_tax' => 'sanitize_string',
				'subtotal' => 'sanitize_string',
	...
	function edit($rec_id = null, $formdata = null){
		...
		$tablename = $this->tablename;
		 //editable fields
		$fields = $this->fields = array("id","user_id","carrier_id","invoice_id","invoice_date","delivery_date","total_discount","tax","shipping","shipping_tax","subtotal","total","creator","slug","status","created_at","updated_at");
		if($formdata){
			$postdata = $this->format_request_data($formdata);
			$this->rules_array = array(
				'user_id' => 'required|numeric',
